...
Les orgues disparus ne sont pas mentionnés à l'ouverture de l'inventaire, puisque comme son nom l'indique il s'intéresse aux instruments actuellement présents sur le territoire. Cependant, rien n'interdit de les renseigner (les évènements de type destruction et disparition sont disponibles pour cela).
...
Chaque orgue est positionné sur une carte et décrit par une fiche.
La [fiche descriptive](fiche-orgue.md) d'un instrument inclut sa composition, les sommiers, la tuyauterie, le vent, etc. et est accompagnée de photos et tout autre type de fichier, comme par exemple des documents d'archives.
...
Le portail de l'inventaire national des orgues a été payé par l'association Orgue en France avec le soutien financier du ministère de la culture, partenaire du projet (directions générales de la création artistique et du patrimoine).

### Qui a réalisé le portail de l'inventaire ?
La [société FabDev](https://fabdev.fr/), dans un esprit collaboratif avec un bénévole d'Orgue en France, et selon les pratiques de l'open source.
...
Le projet d'inventaire national ne mise pas tout sur le numérique, mais un site participatif nous a semblé un bon outil capable de viser une échelle nationale et d'être à jour. En effet, les livres d'inventaire perdent leur exhaustivité quelques années après leur publication, en raison des constructions, relevages et modifications d'instruments.
Les livres déjà édités ne perdent pas moins leur valeur, notamment pour ce qui concerne les données historiques.
Par ailleurs, l'inventaire ne concurrence en rien, voire alimente, les initiatives d'édition de livres d'art dédiés à l'orgue, de brochures locales, etc.

### Où les données déjà présentes à l'ouverture du portail ont-elles été piochées ?
La liste des orgues est en grande partie basée sur les livres d'inventaire (à l'exception de l'Orne, des Ardennes, d'une partie du département du Nord), puis mise à jour manuellement selon les informations qui nous ont été communiquées.
Les [données](donnees.md) présentes à l'ouverture du portail au sein des fiches descriptives ont principalement été récupérées depuis les sites associatifs, avec accord des associations éditrices : nous remercions vivement l'ensemble des associations concernées pour leur aimable participation.
...
Tout un chacun.
Tout utilisateur acceptant la [charte du contributeur](charte-du-contributeur.md) peut demander la création d'un compte et
...
Cela étant, il n'est pas nécessairement souhaitable de dupliquer inutilement une fiche de l'inventaire dès lors qu'elle est similaire. Une juste complémentarité sera certainement à privilégier au cas par cas.
C'est ainsi que certaines associations ont déjà fait part de leur souhait de basculer leur propre inventaire sur l'inventaire national, économisant ainsi des coûts de maintenance et du temps de mise à jour.
...
En revanche fichiers images, sonores ou vidéos peuvent conserver leur droit d'auteur (il est donc possible de conserver le nom de l'auteur dans le fichier ou le nom du fichier) : il s'agit là d'un droit moral.
...
Lorsque vous vous inscrivez sur le site, vous acceptez la [charte du
...
Oui, l'ensemble du projet est réalisé en code ouvert (open source) : il est donc [accessible](code-source.md) et modifiable par quiconque.
